...
11. **文档约定（永久）**：同一大版本（如 `1.3.5`）内的**所有改动都写入 `docs/iWP-S-<大版本>-Dev-记录.md`**，不再按主题另开新文档。每条改动一个二级标题：`## <构建号>-Dev　<提交短哈希>　<一句话目的>　<类型标签>`，下面写本次改动详情（背景 / 改动 / 涉及文件 / 验证 / 风险）。类型标签见该文件头部。**每次推送后必须补上本次构建对应的条目。**
