...
13. **改 CSS 后先跑断点审计**：`npm run audit:breakpoints`（= `node scripts/audit-breakpoints.mjs`）会机械比对「关注点 × 分栏带」（`768–959` 与 `≥960` 必须成对）以及「同一元素两段的 z-index 是否一致」，并可 `--strict` 作为提交前门禁。**新增任何分栏相关规则时，务必让它在这两个带里成对出现** —— 1.3.5.32 / 33 / 35 的返工全部源于"某条规则只写在 ≥960 段里"。
