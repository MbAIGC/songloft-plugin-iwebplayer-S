...
> （已发生：2026-09-12 按用户要求 `1.1.7-dev` → `1.3.2-dev`；2026-09-18 跟进上游 v1.3.5 → `1.3.5-dev`；2026-09-21 跟进上游 v1.3.6 → `1.3.6-dev`）。
...
| dev tag = `dev-<base 去掉 -dev>` | `1.3.6-dev` → `dev-1.3.6`；**同一基础版本共用一个 tag**，每次构建只追加 zip，不删历史资产 |
| Release 标题 = tag（裸格式） | 两个 dev 工作流都写成 `dev-1.3.5`，不出现 `iWebPlayer-S dev v...` 这类标题 |
| **跟进上游时必须同步提升版本号** | **默认规范**：每次跟进上游（如上游发布 v1.3.6）就把 base 版本改为「上游版本 + `-dev`」（`1.3.6-dev`），并同步 `README.md` badge/文件名、`DEV_RELEASE_NOTES.md` 标题与条目、本页版本表；这是跟进的固定动作，不需要再单独确认 |
...
| 位置 | 版本 | 说明 |
| --- | --- | --- |
| `plugin.json` | `1.3.6-dev` | 插件**基础版本**，SongLoft 读取；CI 构建时临时改为 `1.3.6.NN-dev` |
| `package.json` | `1.3.6-dev` | npm 包版本，应与 plugin.json 一致（**APK 工作流有门禁会校验两者一致**）|
| `package-lock.json` | `1.3.6-dev` | 锁文件；无 registry 时直接改顶层与 `packages[""]` **共 2 处** version |
| `static/index.html` | 占位符 `__APP_VERSION__` | **不要手写版本号**：构建期由 `scripts/inject-version-hashes.mjs` 注入实际构建版本 |
| `README.md` badge | `v1.3.6` | Shields.io 徽章 |
| `DEV_RELEASE_NOTES.md` | `dev v1.3.6` | 预发布说明标题，CI 直接读作 release notes |
...
8. **上游同步策略**：上游 [`songloft-org/songloft-plugin-iwebplayer`](https://github.com/songloft-org/songloft-plugin-iwebplayer)
   ...
   **跟进时按默认规范同步提升 base 版本号**（上游 v1.3.6 → `1.3.6-dev`），并同步 README / DEV_RELEASE_NOTES / 本页版本表。
   已有评估与实施记录见 `docs/IWP-1.1.7上游优化与本地跟进评估.md`、`docs/IWP-1.1.8-1.3.2上游跟进评估.md`、`docs/IWP-1.3.3-1.3.5上游跟进记录.md`。
11. **文档约定（永久）**：同一大版本（如 `1.3.5`）内的**所有改动都写入 `docs/iWP-S-<大版本>-Dev-记录.md`**，不再按主题另开新文档。
    **版面固定为**：① 顶部约定说明（blockquote）→ ② `## 速查索引` 表格（构建 / 提交 / 概述 / 类型）→ ③ 按阶段分组（`## 阶段 N · …`）的条目。
    **每条格式**（照此照抄）：三级标题 `### <构建号>-Dev　<提交短哈希>　　<类型标签>`，下面依次三行 —— `**概述**：`（一句话）、`**用户需求**：`（当时要解决什么）、`**改造思路**：`（怎么改的）；末尾一行引用式元信息 `> 日期 ｜ 提交 ｜ 文件：…`，补充说明用 `> ` 引用行贴在后面。
    **每次推送后必须补上本次构建的条目并同步更新索引表**；跨版本关联写 `→ 见 1.3.5.NN`。
13. **改 CSS 后先跑断点审计**：`npm run audit:breakpoints`（= `node scripts/audit-breakpoints.mjs`）会机械比对「关注点 × 分栏带」（`768–959` 与 `≥960` 必须成对）以及「同一元素两段的 z-index 是否一致」，并可 `--strict` 作为提交前门禁。**新增分栏相关元素/规则时，同步把它登记进该脚本的 `CONCERNS` / `Z_TARGETS` 清单**（清单是手工维护的，不登记就守不住）；**新增任何分栏相关规则时，务必让它在这两个带里成对出现** —— 1.3.5.32 / 33 / 35 的返工全部源于"某条规则只写在 ≥960 段里"。
14. **"是不是我们引入的 / 原版行为"的对照基准是上游仓库**：`upstream` = `songloft-org/songloft-plugin-iwebplayer`（已配好 remote）。查看上游源码用 `git fetch upstream` 后 `git show upstream/main:static/index.html`（或 `git ls-remote --tags upstream` 取 tag 的 SHA 再 `git show <sha>:路径`）。
    ⛔ **不要拿我们自己的历史 tag（v1.3.x）当"原版"基准** —— 那是我们自己的产物，会把"我们早就改过的差异"误判为基准。
    例外：分栏/宽屏等**我们独有的功能**（上游没有），其回归只能拿我们自己的历史 tag 对照。
15. **多端适配原则（App / 网页 × 手机竖屏 / 半宽屏 / 宽屏）**：适配只有三条互相独立的轴 —— **① 容器**（App 还是网页，判据 `html.iwp-app`）、**② 宽度带**（`<768` 手机竖屏；`≥768` 进分栏，判据 `body.split-view-active`）、**③ 视口高度**（统一走 CSS 变量 `--vh100`：`:root{--vh100:100vh}` + `@supports(height:100dvh){:root{--vh100:100dvh}}`，**不要**再写裸 `100vh`）。硬性要求：
    - **布局层：App 与网页必须完全一致** —— ⛔ 禁止让 `html.iwp-app` 承载任何**影响布局**的规则（尺寸 / 间距 / 显隐 / 顺序都不行）；
    - **观感层：允许"仅 App"微调**，但必须**四条同时满足**：① 作用域带 `html.iwp-app`；② 只用**不参与布局**的属性（`transform` / `opacity` / `filter`）；③ 用 `clamp()` 设上下限（矮屏不过度、高屏不顶到播放条）；④ 在本大版本记录文档里写明"为什么只有 App 需要"；
    - ⚠️ **"半宽屏"不是断点**：它是"分栏下窗口较窄"的**使用状态**，实测可落在 `≥960`（本机 **1063 CSS px**）。做"仅半宽屏生效"的差异化时**必须用实测宽度**确认，**不要假设 = 768–959**（1.3.6.16 的返工就源于此）；
    - **量纲纪律**：涉及 `vh/dvh/vw/px` 的计算只用 **CSS px**；截图是**物理像素**，须 ÷ `devicePixelRatio`（本机实测 ≈**2.22**，按 2 推算会把 1183 当真实宽度，实际 ≈1063 ✗）；
    - **验收纪律**：判断某条规则"有没有生效"，**优先把数字显示出来**（临时在界面显示视口/DPR/位移量，或读 `getComputedStyle(el).transform`），**不要靠截图估算** —— 1.3.6 的 17 个构建里前 11 个都在"猜"，显示数字后一次定案；
    - 详细判据、各场景差异表、陷阱清单、检查清单与完整时间线见 **`docs/iWP-S-多端适配原则.md`**。

12. **布局状态只允许「一个判据、一个执行者」**：分栏/宽屏判定**只能用** `matchMedia('(min-width: 768px)')`（与 CSS 同源，不受捏合缩放影响）；所有应用（`split-view-active`、工具栏归属、`--split-list-top`）**只能由 `static/index.html` 的 `syncLayout()`** 完成。新增事件源时只调 `syncLayout()`；**禁止**新增散落的 `innerWidth >= 768` 判断；**禁止**用 `MutationObserver` 观察 `body` 的 class 去驱动布局（那是「拿结果当输入」，1.3.5.13–27 的缩放回归正是被它放大的）。
10. ⛔ **绝不允许在输入框聚焦/失焦时改变布局**（隐藏整栏、切换按钮文字宽度、增删元素等）。平板浏览器会为保持光标可见而平移/缩放页面，叠加 `maximum-scale=1.0` + 全屏锁滚动后无法自行恢复，表现为"窗口缩放失效 / 底部黑块"（详见 `docs/分栏模式三栏工具栏改造总结.md` 3.22，1.3.5.13→1.3.5.27 的回归就源于此）。需要空间请用**按宽度的静态媒体查询**压缩，不要用焦点状态驱动布局。
9. **提交前自检**（与 CI 质量门禁对齐）：
