...
11. **文档约定（永久）**：同一大版本（如 `1.3.5`）内的**所有改动都写入 `docs/iWP-S-<大版本>-Dev-记录.md`**，不再按主题另开新文档。每条改动一个二级标题 `## <构建号>-Dev　<提交短哈希>　<一句话目的>　<类型标签>`。
    **版面固定为**：① 顶部约定说明（blockquote）→ ② `## 速查索引` 表格（构建 / 提交 / 目的 / 类型）→ ③ 按阶段用 `> ### 阶段 N · …` 分隔的条目；每条 = 二级标题 + 一行元信息（`` `日期` ｜ `提交` ``）+ `文件：…` + 若干要点列表。
    **每次推送后必须补上本次构建的条目并同步更新索引表**；跨版本关联用 `→ 见 1.3.5.NN` 互相引用，便于按号追溯。
