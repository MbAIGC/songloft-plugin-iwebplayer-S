...
| 19 | `43b6ac2` | 据 GPT 分析修正：区间→**768–1199**、位移加 `clamp`、诊断显示**实际生效像素**；归档分析文档 | 🐞 修复·外观 |
| 20 | `4860e39` | 诊断增强（DPR / 分栏状态 / 是否沉浸页 / 将要生效的 px）| 🔍 诊断 |
| 21 | `5477ae3` | **移除临时诊断**，版本标签恢复干净（半宽屏已生效，定位完成）| 🧹 清理 |
| — | `258811e` | 沉淀**多端适配原则**：详细版入 `docs/iWP-S-多端适配原则.md`，简版入 `AGENT.md`（约定 15）（**仅文档，无构建号**）| 📝 文档 |
| 22 | `e476700` | **CSS 体检**：删 1 条被完全覆盖的规则 + 1 条死规则 + 2 段过时注释 | 🧹 清理 |

> 共 **22** 条改动（另有 2 条仅影响 APK、1 条仅文档，均不产生插件构建号）。
...
### 1.3.6.19-Dev　43b6ac2　　🐞 修复·外观 + 🔍 诊断

**概述**：据 GPT 分析（已归档）修正"单位认知错误 ✗"与"断点区间 ✗"；位移加 `clamp` 上下限；诊断显示实际生效像素

**用户需求**：「@docs/GPT-iWP-S-1.3.6.15-半宽屏App网页Cover位置差异分析.md 看看有没有启发」。

**GPT 分析中可采纳的三点**：
1. **单位错误（我的）** ✗：`calc(var(--vh100) * 0.08)` 里的 `dvh` 是 **CSS px**，而我一直拿截图的 **2400 物理像素** 计算 →
   我报给用户的"≈192px"其实是**物理**像素；**实际 CSS 位移 ≈ 1200 × 8% = 96 CSS px** ✓（用户认可的那个"正好"就是 **96 CSS px** ✓）。
2. **断点不是差异来源** ✓：两张截图推算都是 ≈1180 CSS px（App≈1183 / 网页≈1179）→ 两端都在 ≥960 ✓ →
   我 1.3.6.17 把区间设成 768–1119 ✗ **恰好漏掉 1180** ✗ → 这就是"依旧没解决"的直接原因 ✓。
3. `transform` 不参与布局 → 矮屏存在过移/裁剪风险 ✗ → 建议加 `clamp()` ✓。

**改动**：
- 区间 `768–1119` → **`768–1199`**（覆盖 ≈1180 ✓，避开更宽的"宽屏"✓）；
- 位移加边界：`translateY(clamp(24px, calc(var(--vh100) * 0.08), 140px))`（@H=1200 → **96px** ✓）；
- 诊断标签追加**实际生效像素**（解析 `getComputedStyle(main).transform` 的 matrix 平移量 ✓）：
  形如 `1.3.6.19-dev｜1183×1200｜屏1200｜App｜位移ON(96px)` —— 单位换算从此**不用再靠截图推算** ✓（即 GPT 第九节的运行时验证建议 ✓）。
- GPT 分析文档归档到 `docs/` ✓。

> `2026-09-21` ｜ `fix(ui): widen the immersive nudge band to 768-1199 and clamp it; diag shows the effective px; archive the GPT analysis` ｜ 文件：`static/index.html`、`docs/GPT-iWP-S-1.3.6.15-半宽屏App网页Cover位置差异分析.md`

### 1.3.6.20-Dev　4860e39　　🔍 诊断

**概述**：诊断标签增强 —— 追加 DPR、分栏状态、是否在沉浸页、以及"将要生效的位移像素"

（用户随后实测反馈：**半宽屏生效 ✓** → 定位任务完成。）

### 1.3.6.21-Dev　5477ae3　　🧹 清理

**概述**：移除临时诊断，`☰ → 版本` 恢复为只显示版本号

**用户需求**：「.19，半宽屏生效了」→ 排查收尾 ✓。

**本次排查的最终答案（记录备查）**：
- 实测**半宽屏 = 1063×1078，屏高 1079，DPR≈2.22** ⇒ 半宽屏宽度 = **1063 CSS px** ✓
  （既不是 768–959 ✗，也不是 GPT 文档按 DPR=2 推算的 1183 ✗ —— 其 DPR 假设偏了，但"两端都 ≥960"的结论仍成立 ✓）；
- **生效配置**（最终态）：
  ```css
  @media (min-width: 768px) and (max-width: 1199px) {          /* 覆盖 1063 的半宽屏；更宽的宽屏不命中 */
    html.iwp-app body.split-view-active.player-open .desktop-player-main {
      transform: translateY(clamp(24px, calc(var(--vh100) * 0.08), 140px));   /* ≈86 CSS px @1078 */
    }
  }
  ```
  → 半宽屏（1063 ✓）命中 ✓；宽屏（更宽 ✓）不命中 ✓；**网页端**因 `html.iwp-app` 隔离 ✓ 完全不受影响 ✓。
- 位移实测 ≈ **86 CSS px**（8% × 1078 ✓，clamp 未触边界 ✓）。

**结论**：这是一次"**用运行时数据代替猜测**"的收尾 —— 期间我犯过两个方向性错误 ✗：
① 用**截图物理像素**算 CSS 位移（192px vs 实际 96px ✗）；② 用**猜测的断点区间**去适配实测宽度（1119 漏掉 1063 ✗）。
两者都靠"把尺寸/生效量直接显示在界面上"解决 ✓。

> `2026-09-21` ｜ `chore(ui): remove the temporary viewport diagnostic (half-width nudge confirmed working)` ｜ 文件：`static/index.html`

### （仅文档，无构建号）258811e　　📝 文档

**概述**：沉淀"多端适配原则" —— 详细版入 `docs/iWP-S-多端适配原则.md`（189 行），简版入 `AGENT.md`（关键约定 15）

**用户需求**：「好好总结一下，项目对于 手机 app、网页、手机竖屏、半宽屏、宽屏的适配原则，简要说明写入 AGENTS，详细说明记录在 /docs」。

**说明**：仓库既有约定文件为 `AGENT.md`（单数），本次即写入该文件（未新建 `AGENTS.md` ✓）。

**沉淀内容（详细版章节）**：
1. **五个场景 = 三条轴**：容器（App/网页，`html.iwp-app`）× 宽度带（手机竖屏 `<768` / 分栏 `≥768`，`split-view-active`）× 视口高度（`--vh100`）；
2. **10 条硬性原则**：一条判据一个执行者、两带成对、**布局层 App/网页必须一致**、观感层"仅 App"微调四条约束、`--vh100` 统一口径、不侵入手机态、焦点不驱动布局、**量纲纪律（CSS px vs 物理像素）**、**验收纪律（把数字显示出来，不要猜）**、改完必跑的门禁；
3. **各场景差异表**（分栏/高度/主列/位移/`iwp-app`/`--vh100`/验收重点，逐项对照）；
4. **`html.iwp-app` 四重判据**（JS 桥 / UA `; wv)` / 自定义 UA / 视口≈屏幕高）+ 已知误判风险；
5. **实测数据存档**（半宽屏 1063×1078、屏高 1079、DPR≈2.22、位移 ≈86 CSS px、生效区间 768–1199）；
6. **8 条陷阱清单**（含 `translateY(%)` 基准、物理 vs CSS 像素、断点≠场景、`display:contents` 上 transform 无效、`transform` 不参与布局、`dvh` 回退、`immutable` 子资源、别只用浏览器验收）；
7. **9 项新增规则检查清单**；
8. **1.3.6.05 → 1.3.6.21 时间线 + 最大教训**：*能显示的数字，不要去猜。*

> `2026-09-21` ｜ `docs: codify the multi-form-factor adaptation principles (AGENT.md convention 15 + detailed doc)` ｜ 文件：`docs/iWP-S-多端适配原则.md`、`AGENT.md`

### 1.3.6.22-Dev　e476700　　🧹 清理（CSS 体检）

**概述**：对 1.3.6 周期（17 个构建）的改动做机械体检 —— 删除 1 条**被完全覆盖**的规则、1 条**死**规则、2 段**过时注释**

**用户需求**：「有没有前面改的多余 无效代码，需要删除的？」

**方法**：脚本扫描（① 带**媒体上下文**的重复声明；② CSS 引用但 HTML/JS 中不存在的 id/class；③ 与现状不符的注释）+ 人工复核。

**扫描结论：真冗余很少** ✓
| 项 | 结论 |
|---|---|
| "完全重复声明" | 仅 3 组，其中 2 组是 `@keyframes` 的同名步骤（**误报** ✓）|
| 同一选择器+属性重复 | 14 组，绝大多数是 `-webkit-` 前缀 / 不同媒体段的正常覆盖 ✓ |
| CSS 引用的 id | 9 个里 8 个是**十六进制颜色**被误判 ✓；**1 个真死** ✓ |
| CSS 引用的 class | 57 个绝大多数由 JS **动态创建**（`.eq-bar`←`miot.js`、`.edit-pl-*`←`playlist.js`/`plugins.js` 等 ✓）→ 无需处理 ✓ |

**删除项**：
1. `body.ambient-active.player-open .full-player { top:0; padding-top:56px }` —— 同选择器的后续规则（同为全局、带 `!important`）**已完整覆盖**这两个声明 ✗ → 整条删除 ✓；
2. `#mf-search-playlist-btn { border-*-radius: 8px }` —— 全仓（HTML + 所有 JS）**无任何消费者** ✗ → 删除 ✓；
3. 两段**过时注释** ✗：① 旧的"封面 `34vw` → 半宽屏只有 261–326px"理论；② 描述**已移走**那条位移、且含**作废的单位推算**（1.5%/36px/192px）的注释 → 改写 / 清除 ✓。

**特意保留（附理由）**：
- `768–959` 段沉浸页的"确定高度 + 垂直居中 + 5% 视口内边距"：服务**真实落在 768–959 的窗口** ✓，且与 ≥960 段**成对** ✓（断点审计要求 ✓）—— 实测用户的"半宽屏"是 **1063px（属 ≥960）** ✓，本就不走这一段 ✓；
- App 判据的**第 4 条**（视口≈屏幕高）：有"全屏浏览器误判"风险 ✗，但无法确认前三条是否已足够 ✓ → **暂留**（风险已记录 ✓）；
- 两处"仅 App"位移：用户实测有效 ✓ → 保留 ✓。

> `2026-09-21` ｜ `chore(css): remove a fully-shadowed rule and a dead rule, and clean up stale comments` ｜ 文件：`static/index.html`
