...
- [ ] 1. 清除不可信数据到 `innerHTML` 的持久型 XSS
  - 业务文本一律 `textContent` / `createElement` / `append` / `replaceChildren`；URL 用属性赋值并限制协议；移除动态内联事件改用监听器；文本/属性/URL 分上下文处理，不能只靠一个 `escapeHtml`
  - 涉及：`static/lyrics.js`（歌词/KTV）、`utils.js`（Toast）、`miot.js`（设备名）、`plugins.js`（插件名/版本/平台）、`playlist.js:936-944,1188-1199`（歌单/歌曲/来源标签）、`online.js`（在线歌单/详情/WebDAV 目录名）、`player.js:203`（当前歌曲）
...
- [ ] 2. 限制 Android WebView 导航与 Bridge 来源
  ...
  - 涉及：`MainActivity.java:126,146-205,205,686-737`
...
- [ ] 18. 两个 dev 工作流（APK/插件）同改一个 Release → 共享 concurrency group 或单一流程发布
  - 涉及：`.github/workflows/build-apk-dev.yml`、`build-plugin-dev.yml`
- [ ] 19. 发布版本计算：`fetch-depth: 0`；稳定版只选非 prerelease `vX.Y.Z` 且单调递增；说明按标签范围生成
