...
**问题：** 最近的 dev 调整中，部分界面体验修复尚未记录在本方案文档：音箱推送进度提示完成后残留、横屏播放页缩放时左侧歌曲信息区域尺寸过于固定、时间显示与控制栏间距不够协调，以及首页右侧倍速按钮与左侧收藏按钮的边缘距离不一致。

**实现记录：**

1. `static/player.js`：音箱虚拟列表推送成功后，用普通成功 Toast 覆盖持久的“正在将列表打包推送到音箱...”提示，避免任务完成后仍显示进行中。
2. `static/index.html`：桌面/平板横屏播放页的歌曲信息区域保持约 32%，歌词区域使用剩余约 68%；左侧区域、封面、间距和 padding 允许随窗口缩放，避免窗口变窄时只压缩歌词区域。手机竖屏规则未修改。
3. `static/index.html`：播放时间和总时长保持左右分布，字号从 11px 调整为 13px，桌面/平板播放页进一步使用 14px；时间行取消桌面播放页原有的 960px 宽度限制，使其更接近进度条左右端。
4. `static/index.html`：首页横屏状态只给右侧倍速按钮补充右侧 10px 视觉边距，收藏按钮位置保持不变。
