...
### 1.3.5.45-Dev　079aaeb　　🐞 修复·外观

**概述**：深色模式下手机沉浸（歌词）页改为与浅色一致（全页跟随封面取色），删除此前加的顶栏遮罩

**用户需求**：提供两张对比截图（`170934` 正常 / `171313` 问题），要求**先理解、排查**；随后选定**方案 ②** —— 深色下沉浸页与浅色保持一致、不额外压暗。

**像素级排查（本次的定位方法，值得复用）**：手工解码两张 PNG（1222×2078 / 1220×2076，2× DPR）后沿左边缘逐行取样：
| 位置 | 170934（浅色，正常） | 171313（深色，问题） |
| --- | --- | --- |
| 状态栏/顶栏 y=10/41/103 | `(220,215,186)` 米色 | `(99,101,98)` / `(98,100,97)` 深灰 ✗ |
| 顶栏下 y=186 | `(220,215,186)` | `(169,168,150)` 中灰 |
| 中部 y=415–1370 | `(220,215,186)` | `(220,215,186)` ✓ 相同 |
| 底部 y≥1494 | `(220,215,186)` | `(169,168,150)` 中灰 |

按 alpha 反推即锁定规则：
- `0.4×220 + 0.6×17 = 98` → **正是我在 1.3.5.43/44 加的 `rgba(17,24,39,0.6)` 顶栏遮罩**（方向改反了：让顶栏变成全页最暗的一块）；
- `0.75×220 + 0.25×17 ≈ 169` → **上游暗色规则的 `rgba(17,24,39,0.25)`**（作用于 `.full-player` / `.player-bar`）。
→ 结论：深色下三块用了**三种不同处理**（0.6 / 0.25 / 不处理）才是"分裂"的根源；我上一版把"顶栏发白"当成强度问题，其实**强度越小越接近**。

**修法（方案 ②，用户选定）**：
- **删除顶栏遮罩**（1.3.5.43 / 44 两版实验作废）；
- 深色下对**手机沉浸页**把上游暗化**恢复为浅色模式的值**：`.full-player`/`.player-bar` → `rgba(255,255,255,0.08)` + `blur(20px)`；画报遮罩 → `rgba(255,255,255,0.35)`；封面图 → `opacity:0.28` + `filter:none`；`#device-val`/`.fp-corner-btn` → `rgba(255,255,255,0.18)`。
...
- **⑬ 上游 v1.3.6 跟进评估（待决策）**：上游已发布 `v1.3.6`（`v1.3.5 (21b4615)` → `v1.3.6 (d320183)`，2 提交 / 7 文件 / +254 −28），内容为：A 图标重构、B 头部导航图标（在线资源 / 曲库搜索）、C 列表过滤与跳转源、D 版本与缓存串。逐项可适配性、与分栏改造的 3 处冲突、以及"v1.3.5 的本地过滤我们可能漏了"的发现，详见 **`docs/IWP-1.3.5-1.3.6上游跟进评估.md`**。结论：**不能 merge/cherry-pick，需手工移植**；建议先做低风险的 A+B，C 需先定"分栏下这些控件放哪"；若跟进则按规范把 base 升到 `1.3.6-dev`。**状态：仅评估，未改代码（2026-09-19）。**
